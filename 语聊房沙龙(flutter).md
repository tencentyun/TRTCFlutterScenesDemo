...
[源码](https://github.com/tencentyun/TRTCSDK/tree/master/Android/TRTCScenesDemo/trtcvoiceroomdemo/src/main/java/com/tencent/liteav/trtcvoiceroom) 中的 trtcchatsalondemo 文件夹包含两个子文件夹 ui 和 model，model 文件夹中包含可重用的开源组件 TRTCChatSalon，您可以在 `TRTCChatSalon.dart` 文件中看到该组件提供的接口函数，并使用对应接口实现自定义 UI 界面。
...
5. 进房后会收到组件的 `TRTCChatSalonDelegate.onAudienceEnter`  、  `TRTCChatSalonDelegate.onAnchorEnterMic`  、 `TRTCChatSalonDelegate.onAnchorLeaveMic`  等等变化事件通知，此时可以记录房间属性并做相应改变，例如 UI 展示房间名、记录上麦是否需要请求主播同意等。
6. 进房后会收到组件的 `TRTCChatSalonDelegate.onAudienceEnter` 和 `TRTCChatSalonDelegate.onAudienceExit` 当前房间主播信息回调，此时可以根据麦位列表的信息查询到当前房间主播的用户信息，然后刷新到 UI 界面上。
7. 进房后还会收到麦位表有主播进入的 `TRTCChatSalonDelegate.onAnchorEnterMic` 和 `TRTCChatSalonDelegate.onAnchorLeaveMic` 的事件通知。
...
#### 观众主动申请上麦
1. 观众端调用 `raiseHand` ,通过 `sendC2CCustomMessage` 传入主播的 OwnerUserId 和业务的自定义命令字 `raiseHand` 。
